"""
Ablation study for ODE-GS on the D-NeRF synthetic dataset.
...
Default scene: jumpingjacks (D-NeRF synthetic Blender dataset).
Outputs land in:
  output_ablation_dnerf/<variant>/<scene>/

Usage:
  python ablation_dnerf.py                          # run all 5 variants on jumpingjacks
  python ablation_dnerf.py --scenes bouncingballs lego
  python ablation_dnerf.py --variants full no_A no_omega
  python ablation_dnerf.py --skip_existing          # skip already-finished combos (default ON)
  python ablation_dnerf.py --no_skip_existing       # re-run everything
"""
...
def output_dir(variant_name, scene):
    return os.path.join("output_ablation_dnerf", variant_name, scene)
...
# ---------------------------------------------------------------------------
def main():
    parser = argparse.ArgumentParser(description="ODE-GS ablation study on D-NeRF")
    parser.add_argument(
        "--scenes", nargs="+", default=ALL_SCENES, choices=ALL_SCENES,
        metavar="SCENE",
        help="D-NeRF scenes to evaluate (default: all scenes)",
    )
    parser.add_argument(
    ...
    for variant in selected_variants:
        ...
        for scene in selected_scenes:
            ...
            #   - white_background=True  (GT images are white-background composited)
            #   - load2gpu_on_the_fly    (handles larger scenes)
    ...
    print(f"\nOutputs are in:  output_ablation_dnerf/<variant>/<scene>/")
    print("All done.\n")
